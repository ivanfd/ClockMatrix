        ...
        case KEY_UP_EVENT:
            ...
            if (dst_flag) {// якщо час літній
                if (TSTime.Thr == 0)
                    ...
                    setTime((unsigned)(TSTime.Thr - 1), TSTime.Tmin, 0);
            } else
                setTime(TSTime.Thr, TSTime.Tmin, 0);
            break;
        case KEY_DOWN_EVENT:
...
if(en_put)
{
    ...
    putchar_b_buf(18,(unsigned)type_font % 10, pFont, Dis_Buff);
...
if(en_put)
{
    ...
    putchar_b_buf(6,STR_TP_CLK[1], (void *)Font, Dis_Buff);
        ...
        putchar_b_buf(18, (unsigned)type_clk % 10, pFont, Dis_Buff);
        putchar_b_buf(24, 0, (void *)Font, Dis_Buff);
    }
    Update_Matrix(Dis_Buff); // обновити дані на дисплеї
    en_put = 1;
...
void brg_set() {
    switch (events) {
        ...
        case KEY_UP_EVENT:
            ...
            RTOS_SetTask(default_state, 2000, 0);  // 10 секунд для виходу
            events = MAIN_EVENT;
            write_eep(EE_TYPE_BRG,(uint8_t)brg_type);
            break;
        case KEY_DOWN_EVENT:
            brg_type = !brg_type;
            RTOS_SetTask(default_state, 2000, 0);  // 10 секунд для виходу
...
if(en_put)
{
    ...
    putchar_b_buf(6,STR_BRG[1], (void *)Font, Dis_Buff);
    putchar_b_buf(12,STR_BRG[2], (void *)Font, Dis_Buff);
    putchar_b_buf(18,(unsigned)brig % 10, (void *)pFont, Dis_Buff);
    putchar_b_buf(24,0, (void *)Font, Dis_Buff);
}    
       Update_Matrix(Dis_Buff);          // обновити дані на дисплеї
       en_put=1;    
...
if(en_put)
{
    ...
    putchar_b_buf(18,(unsigned)en_h_snd % 10, (void *)pFont, Dis_Buff);
    putchar_b_buf(24,0, (void *)Font, Dis_Buff);
}    
       Update_Matrix(Dis_Buff);          // обновити дані на дисплеї
       en_put=1;    
...
switch (events)
   {
        ...
        case KEY_UP_EVENT:
            ...
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
            events = MAIN_EVENT;
            write_eep(EE_EN_DS1, (uint8_t)en_ds_1);
            break;
        case KEY_DOWN_EVENT:
            en_ds_1 = !(en_ds_1);
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
...
switch (events)
   {
        ...
        case KEY_UP_EVENT:
            ...
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
            events = MAIN_EVENT;
            write_eep(EE_EN_DS2, (uint8_t)en_ds_2);
            break;
        case KEY_DOWN_EVENT:
            en_ds_2 = !(en_ds_2);
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
...
switch (events)
   {
        ...
        case KEY_UP_EVENT:
            ...
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
            events = MAIN_EVENT;
            write_eep(EE_EN_BMP, (uint8_t)en_bmp280);
            break;
        case KEY_DOWN_EVENT:
            en_bmp280 = !(en_bmp280);
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
...
switch (events)
   {
        ...
        case KEY_UP_EVENT:
            ...
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
            events = MAIN_EVENT;
            write_eep(EE_EN_DST, (uint8_t)en_dst);
            break;
        case KEY_DOWN_EVENT:
            en_dst = !(en_dst);
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
 ...
 switch (events)
   {
        ...
        case KEY_UP_EVENT:
            ...
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
            events = MAIN_EVENT;
            write_eep(EE_EN_AM2302, (uint8_t)en_am2302);
            break;
        case KEY_DOWN_EVENT:
            en_am2302 = !(en_am2302);
            RTOS_SetTask(default_state, 2000, 0); // 10 секунд для виходу
